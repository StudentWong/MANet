...
from options.options_IA_dim import opts
...
def forward_samples(model, image1, image2, samples, out_layer='conv3'):
    ...
    for i, regions1 in enumerate(extractor1):
        for j, regions2 in enumerate(extractor2):
            if i == j:
                # regions1 = torch.tensor(regions1)
                # regions2 = torch.tensor(regions2)
                if opts['use_gpu']:
...
def train(model, criterion, optimizer, pos_feats, neg_feats, maxiter, in_layer='fc4'):
    ...
    for iter in range(maxiter):
        ...
        # create batch
        batch_pos_feats = torch.tensor(pos_feats.index_select(0, pos_cur_idx))
        batch_neg_feats = torch.tensor(neg_feats.index_select(0, neg_cur_idx))
...
def run_mdnet(img_list1, img_list2, init_bbox, gt=None, savefig_dir='', display=False):
    ...
    # Init model
    model = MDNet(opts['stage'], opts['model_path1'])
    if opts['use_gpu']:
        model = model.cuda()
...
def get_sequence(seq, seq_home):
    ...
    img_dir1 = os.path.join(seq_home, seq, visible_img_dir)
    img_dir2 = os.path.join(seq_home, seq, infrared_img_dir)
    gt_path = os.path.join(seq_home, seq, gt_txt)
    ...
    # print(gt)
    # exit()
    result_dir = os.path.join(res_dir)
...
if __name__ == "__main__":
    ...
    for i in range(n_seq):
        ...
        if 'MANet311-2IC_' + seq + '.txt' not in os.listdir(res_dir):
            ...
            # Run tracker
            result, result_bb, fps = run_mdnet(img_list1, img_list2, init_bbox, gt=gt, savefig_dir='',
