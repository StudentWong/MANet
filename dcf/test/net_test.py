...
class DCFNetFeature(nn.Module):
    def __init__(self, model_path=None, layer=3):
        ...
        if (not model_path is None) and os.path.splitext(self.model_path)[1] == '.mat':
...
class DCFNet(nn.Module):
    ...
    def forward(self, xr, xt):
        x = self.feature(xr, xt) * self.config.cos_window
        # x = self.feature(x)
    ...
    def update(self, zr, zt, lr=0.8):
        z = self.feature(zr, zt) * self.config.cos_window
        # z = self.feature(z)
